...
export default function MediaPage() {
  ...
  const loadMediaFiles = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.error('Media API error:', result.error);
      }
    } catch (error) {
  ...
  const handleFileUpload = async (files: FileList) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const result = await response.json();
        if (!result.success) {
          toast.error(`${t.failedToUpload} ${file.name}: ${result.error}`);
        }
      }

      loadMediaFiles();
      toast.success(language === 'bg' ? 'Качването завърши' : 'Upload complete');
    } catch (error) {
  ...
  const confirmDeleteFile = async () => {
    ...
    const path = filePendingDelete;
    const previousFiles = mediaFiles;
    ...
    await optimisticMutate({
      ...
      rollback: () => setMediaFiles(previousFiles),
      request: async () => {
        ...
        const result = await response.json();
        if (!result.success) {
          throw new Error(result.error || t.deleteFileError);
        }
        return result;
      },
      onSuccess: () => toast.success(language === 'bg' ? 'Файлът е изтрит' : 'File deleted'),
      onError: (err) => toast.error(err instanceof Error ? err.message : t.deleteFileError),
    });
  };
  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminConfirmDialog
        ...
        cancelLabel={language === 'bg' ? 'Отказ' : 'Cancel'}
        variant="danger"
      />
                  ...
                  {paginatedFiles.map((file, index) => (
                    <div key={index} className="relative group aspect-square rounded-lg overflow-hidden border border-gray-200">
                      ...
                      <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-opacity flex items-center justify-center">
                        <button
                          onClick={() => setFilePendingDelete(file.path)}
